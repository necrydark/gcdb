...
import { Character, Comment, UserRole } from "@prisma/client";
...
export default function CommentCard({ idx, comment, userId, userRole }: Props) {
  ...
  return (
    <Card key={idx} className={`border-0 rounded-[5px]`}>
      <CardHeader className="flex flex-row justify-between">
        <div>
          <CardTitle className="flex text-white flex-row gap-2">
            <h1> {comment.character.name}</h1>・ <p>{comment.character.tag}</p>
          </CardTitle>
        ...
        {(comment.userId === userId ||
          ...
          <DropdownMenu modal={false}>
            <DropdownMenuTrigger
              className="hover:bg-purple-950 dark:hover:bg-purple-950 rounded-[5px]"
              asChild
            >
      ...
      <CardContent>
        ...
        <Link href={`/characters/${comment.character.slug}`}>
          <Button className="mt-4 rounded-[5px]">View</Button>
        </Link>
      </CardContent>
