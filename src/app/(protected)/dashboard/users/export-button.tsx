...
import { User } from "./columns"; // Make sure to adjust the import path as needed
...
const ExportButton = ({ data }: ExportButtonProps) => {
  const handleExport = () => {
    // Convert data to CSV
    const headers = ["id", "name", "username", "email", "role", "isTwoFactorEnabled"];
    const csvContent = [
      headers.join(","),
      ...data.map(item => 
        headers.map(header => {
          // Handle commas and quotes in the data to ensure proper CSV format
          const value = item[header as keyof User];
