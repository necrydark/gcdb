...
import { HolyRelic } from "./columns"; // Make sure to adjust the import path as needed
...
const ExportButton = ({ data }: ExportButtonProps) => {
  const handleExport = () => {
    // Convert data to CSV
    const headers = ["name", "name", "effect", "attack", "defense", "hp", "beast"];
    const csvContent = [
      headers.join(","),
      ...data.map(item => 
        headers.map(header => {
          // Handle commas and quotes in the data to ensure proper CSV format
          const value = item[header as keyof HolyRelic];
